...
data_path=$1
COG_FLAGS='-co TILED=YES -co COPY_SRC_OVERVIEWS=YES -co COMPRESS=DEFLATE'
...
for sceneid in `tail -n+2 ${data_path}/*.csv | cut -d, -f1`; do
  ...
  mkdir out/${sceneid};
  gdal_translate -b ${bandCounter} ${data_path}/*.tif out/${sceneid}/${sceneid}_Gamma0_VV.tif ${COG_FLAGS}
  bandCounter=$((${bandCounter} + 1))
  gdal_translate -b ${bandCounter} ${data_path}/*.tif out/${sceneid}/${sceneid}_Gamma0_VH.tif
  bandCounter=$((${bandCounter} + 1))
